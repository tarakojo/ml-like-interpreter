...
= struct
  ...
  let rec remove x s = 
    match s with
    | Leaf -> s 
    | Node((y, v), l, r) when Order.compare x y = LT -> Node((y, v), remove x l, r) 
    | Node((y, v), l, r) when Order.compare x y = GT -> Node((y, v), l, remove x r)
    | Node((y, _), l, Leaf) -> l 
    | Node((y, _), Leaf, r) -> r 
    | Node((y, _), Node((z, v), ll, lr), r) -> Node((z, v), remove z (Node((z, v), ll, lr)), r) 
